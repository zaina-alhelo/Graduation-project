...
@section('title-page', 'Edit Appointment')
        ...
        <div class="card-body">
            ...
            <form action="{{ route('appointments.update', $appointment->id) }}" method="POST" id="appointmentForm">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="date" id="date" class="form-control" placeholder="Select date" value="{{ old('date', $appointment->date) }}">
                    @error('date')
                        <div class="text-danger mt-1">{{ $message }}</div>
                    @enderror
                ...
                <div class="mb-3">
                    ...
                    <textarea name="notes" id="notes" class="form-control" rows="3" placeholder="Add any special notes or instructions">{{ old('notes', $appointment->notes) }}</textarea>
                </div>
...
@section('scripts')
    ...
    // تحديد التاريخ من القيمة القديمة إذا كانت موجودة
    const initialDate = "{{ old('date', $appointment->date ?? '') }}"; // التحقق من التاريخ المختار في الصفحة السابقة أو تعيينه إلى القيمة الحالية في المتغير $appointment
    if (initialDate) {
    ...
    // Adding the formatTime function
    function formatTime(time24) {
        const [hours, minutes] = time24.split(':');
        let period = 'AM';
        let hour = parseInt(hours, 10);
